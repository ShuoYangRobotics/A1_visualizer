...
void Builder::Build(MarkerArray* marker_array, int no_color = 0) {
  if (!has_initialized_) {
    ROS_ERROR(
        "You must call Init() before calling any other methods of "
        "robot_markers::Builder.");
  ...
  for (size_t i = 0; i < links.size(); ++i) {
    ...
    Marker marker;
    BuildMarker(*link_p, i, &marker, no_color);
    // std::cout << "reach " << __FILE__ << __LINE__ << std::endl;
...
void Builder::BuildMarker(const urdf::Link& link, int id, Marker* output, int no_color = 0) {
  ...
  if (no_color == 0) {
    ...
    output->color.r = 0.33+0.5*(no_color-1);
    output->color.g = 0.17+0.5*(no_color-1);
    output->color.b = 0.45+0.5*(no_color-1);
    output->color.a = 1.00;
  }
