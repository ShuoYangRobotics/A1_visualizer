...
void ForwardKinematics::GetTransforms(
  ...
  for (std::map<std::string, double>::const_iterator jnt =
    ...
    // ROS_INFO("Robot joint name \"%s\"", jnt->first.c_str());
    ...
    if (seg != segments_.end()) {
      ...
      // ROS_INFO("Robot header link name \"%s\"", seg->second.root.c_str());
      // ROS_INFO("Robot child link name \"%s\"", seg->second.tip.c_str());
